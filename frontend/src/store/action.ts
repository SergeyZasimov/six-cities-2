...
import { adaptNewCommentToServer, adaptNewOfferToServer, adaptRegisterToServer } from '../adapters/adaptersToServer';
...
export const editOffer = createAsyncThunk<void, Offer, { extra: Extra }>(
  ...
  async ( offer, { extra } ) => {
    const { api, history } = extra;
    const { data } = await api.patch<Offer>(`${ApiRoute.Offers}/${offer.id}`, adaptNewOfferToServer(offer));
    history.push(`${AppRoute.Property}/${data.id}`);
  });
...
export const registerUser = createAsyncThunk<void, UserRegister, { extra: Extra }>(
  ...
  async ( { email, password, name, avatar, isPro }, { extra } ) => {
    const { api, history } = extra;
    const { data } = await api.post<{ id: string }>(ApiRoute.Register, adaptRegisterToServer({
      email,
      password,
      name,
      isPro,
...
export const postComment = createAsyncThunk<Comment[], CommentAuth, { extra: Extra }>(
  ...
  async ( { id, comment, rating }, { extra } ) => {
    const { api } = extra;
    const { data } = await api.post<CommentDto[]>(`${ApiRoute.Comments}`, adaptNewCommentToServer({
      comment,
      rating,
      id,
    }));
